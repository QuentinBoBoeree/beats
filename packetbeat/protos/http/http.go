...
func (http *httpPlugin) messageComplete(
	...
	st.message.lastSeq = st.LastSeq
	st.message.lastAck = st.LastAck
	st.message.FirstSeq = st.FirstSeq
	st.message.FirstAck = st.FirstAck
	logp.Info("message.lastSeq:%d-------message.lastAck:%d-------message.firstSeq:%d--------message.firstAck:%d", st.LastSeq, st.LastAck, st.FirstSeq, st.FirstAck)
...
func (http *httpPlugin) correlate(conn *httpConnectionData) {
	...
	for !conn.responses.empty() && !conn.requests.empty() {
		requ := conn.requests.first()
		resp := conn.responses.first()
		logp.Info("req lastSeq:%d----req lastAck:%d", requ.lastSeq, requ.lastAck)
		logp.Info("resp firstSeq:%d----resp firstAck:%d", resp.FirstSeq, resp.FirstAck)

		//需要具体判断中间丢包的是请求还是响应
		if requ.lastSeq != resp.FirstAck || requ.lastAck != resp.FirstSeq {
			if resp.FirstAck > requ.lastSeq {
				logp.Warn("---------detected response loss packets---------")
				requ = conn.requests.pop()
				resp = nil
			} else if resp.FirstAck < requ.lastSeq {
				if isDebug {
					logp.Warn("---------detected request loss packets---------")
				}
				resp = conn.responses.pop()
				requ = nil
			}
